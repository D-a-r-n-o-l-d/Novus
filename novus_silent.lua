...
task.spawn(function()
    ...
    while true do
        ...
        if not DetectedNeutered then
            attempts = attempts + 1
            if attempts > 3 then
                scanPrint("[Novus] Adonis not found after 3 scans — stopping")
                break
            end
            scanPrint(string.format("[%s] Scanning... (%d/3)", os.date("%H:%M:%S"), attempts))
...
---------------------------------------------------------------------
-- SILENT AIM: Original Parvus 2 implementation
---------------------------------------------------------------------
...
RunService.RenderStepped:Connect(function()
    ...
    SilentTarget = GetClosest(true,
        Config.Silent.TeamCheck,
        Config.Silent.VisibilityCheck,
        Config.Silent.DistanceCheck,
        Config.Silent.DistanceLimit,
        Config.Silent.FOVRadius,
        Config.Silent.PriorityList[Config.Silent.PriorityIndex],
        AimbotBodyParts,
...
OldIndex = hookmetamethod(game, "__index", function(self, index)
    ...
    if Config.Silent.Enabled and SilentTarget and math.random(100) <= Config.Silent.HitChance then
        if self == Mouse then
            if index == "Target" then return SilentTarget[3]
            elseif index == "Hit" then return SilentTarget[3].CFrame end
        end
    end
        ...
        if self == Workspace then
            if method == "Raycast" then
                ...
                if typeof(origin) == "Vector3" then
                    args[2] = SilentTarget[3].Position - origin
            ...
            or method == "FindPartOnRayWithWhitelist" then
                if typeof(args[1]) == "Ray" then
                    args[1] = Ray.new(args[1].Origin, SilentTarget[3].Position - args[1].Origin)
                    return OldNamecall(self, unpack(args))
                end
            end
        elseif self == Camera then
            if method == "ScreenPointToRay" or method == "ViewportPointToRay" then
                return Ray.new(SilentTarget[3].Position, SilentTarget[3].Position - Camera.CFrame.Position)
            elseif method == "WorldToScreenPoint" or method == "WorldToViewportPoint" then
                args[1] = SilentTarget[3].Position
                return OldNamecall(self, unpack(args))
            end
        end
    end
...
RunService.RenderStepped:Connect(function()
    local mousePos = UIS:GetMouseLocation()
    if (Config.Aimbot.Enabled or Window.Enabled) and Config.Aimbot.FOVRadius > 0 and Config.AimVisuals.ShowAimbotFOV then
    ...
    if (Config.Silent.Enabled or Window.Enabled) and Config.Silent.FOVRadius > 0 and Config.AimVisuals.ShowSilentFOV then
    ...
    if (Config.Trigger.Enabled or Window.Enabled) and Config.Trigger.FOVRadius > 0 and Config.AimVisuals.ShowTriggerFOV then
...
do
    ...
    Silent:AddToggle('SilentEnabled', { Text = 'Enabled', Default = Config.Silent.Enabled })
    Toggles.SilentEnabled:AddKeyPicker('SilentKey', { Default = 'None', Mode = 'Hold', SyncToggleState = false, Text = 'Silent Aim' })
    local SilentDep = Silent:AddDependencyBox()
    SilentDep:AddToggle('SilentTeamCheck', { Text = 'Team Check', Default = Config.Silent.TeamCheck })
    SilentDep:AddToggle('SilentDistCheck', { Text = 'Distance Check', Default = Config.Silent.DistanceCheck })
    SilentDep:AddToggle('SilentVisCheck', { Text = 'Visibility Check', Default = Config.Silent.VisibilityCheck })
    SilentDep:AddToggle('SilentPred', { Text = 'Prediction', Default = Config.Silent.Prediction })
    SilentDep:AddDropdown('SilentPriority', { Text = 'Aimpoint', Default = 'Closest', Values = {'Closest', 'Head', 'HumanoidRootPart', 'Random'} })
    SilentDep:AddInput('SilentHitChance', { Default = tostring(Config.Silent.HitChance), Numeric = true, Finished = true, Text = 'Hit Chance %', Placeholder = tostring(Config.Silent.HitChance) })
    SilentDep:AddInput('SilentFOV', { Default = tostring(Config.Silent.FOVRadius), Numeric = true, Finished = true, Text = 'FOV Radius', Placeholder = tostring(Config.Silent.FOVRadius) })
    SilentDep:AddInput('SilentDist', { Default = tostring(Config.Silent.DistanceLimit), Numeric = true, Finished = true, Text = 'Distance Limit', Placeholder = tostring(Config.Silent.DistanceLimit) })

    local function wireSilent()
        local c = Config.Silent
        c.Enabled = Toggles.SilentEnabled.Value
        c.TeamCheck = Toggles.SilentTeamCheck.Value
        c.DistanceCheck = Toggles.SilentDistCheck.Value
        c.VisibilityCheck = Toggles.SilentVisCheck.Value
        c.Prediction = Toggles.SilentPred.Value
        c.PriorityIndex = table.find({'Closest', 'Head', 'HumanoidRootPart', 'Random'}, Options.SilentPriority.Value) or 1
        c.PriorityList = {'Closest', 'Head', 'HumanoidRootPart', 'Random'}
        c.HitChance = tonumber(Options.SilentHitChance.Value)
        c.FOVRadius = tonumber(Options.SilentFOV.Value)
        c.DistanceLimit = tonumber(Options.SilentDist.Value)
    end
    for _, ev in ipairs({Toggles.SilentEnabled, Toggles.SilentTeamCheck, Toggles.SilentDistCheck, Toggles.SilentVisCheck, Toggles.SilentPred, Options.SilentPriority, Options.SilentHitChance, Options.SilentFOV, Options.SilentDist}) do
        ev:OnChanged(wireSilent)
    end
end
...
MenuGroup:AddButton('Unload / Reload', function()
    ...
    pcall(function() TriggerFOVCircle:Remove() end)
